...
apply_smtp_settings() {
	PATH_TO_PROPS=$CATALINA_HOME/webapps/jasperserver/WEB-INF
	MAIL_PROPERTIES=$PATH_TO_PROPS/js.quartz.properties
	MAIL_XML=$PATH_TO_PROPS/applicationContext-report-scheduling.xml
...
apply_security_settings() {
	PATH_TO_PROPS=$CATALINA_HOME/webapps/jasperserver/WEB-INF/classes/esapi/
	SEC_CONF_PROPS=$PATH_TO_PROPS/security-config.properties
	VALIDATOR_PROPS=$PATH_TO_PROPS/validation.properties

	# Permanent change
	sed -i -r "s|^(security.validation.sql.on=).*$|\1false|" $SEC_CONF_PROPS
	sed -i -r "s/\(select\|call\)/\(select\|call|with\)/" $VALIDATOR_PROPS
	sed -i -r "/Validator.ValidSQL=/ s|#\s*||; " $VALIDATOR_PROPS
}
